        ...
        <div class="col-md-4 order-md-2 mb-4">
            ...
            <ul class="list-group mb-3">
                ...
                <li class="list-group-item d-flex justify-content-between lh-condensed">
                    <div>
                        <h6 class="my-0">{{$cart_product->name}} x {{$cart_product->quantity}}</h6>
                    </div>
                    <span class="text-muted mx-2">{{$cart_product->price}}</span>
                </li>
                    @php($sum = $sum + ( $cart_product->price * $cart_product->quantity))
                @endforeach
                <li class="list-group-item d-flex justify-content-between">
                    <span>Tax (BDT)</span>
                    <strong>{{$tax = $sum * .15}} TK</strong>
                </li>
                <li class="list-group-item d-flex justify-content-between">
                    <span>Shipping (BDT)</span>
                    <strong>{{$shipping = 250}} TK</strong>
                </li>
                <li class="list-group-item d-flex justify-content-between">
                    <span>Total (BDT)</span>
                    <strong>{{$total = $tax + $shipping + $sum}} TK</strong>
                </li>
            </ul>
        </div>
            ...
            <form action="{{ url('/pay') }}" method="POST" class="needs-validation">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="address" name="address" placeholder="1234 Main St"
                           value="{{$delivery_address}}" required>
                    <div class="invalid-feedback">
